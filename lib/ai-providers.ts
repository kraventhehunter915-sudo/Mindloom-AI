...
export async function runExternalAssistant({ provider, model, apiKey, action, title, content }: AISettings & { apiKey: string; action: AssistantAction; title: string; content: string }) {
  ...
  if (provider === "openai") {
    response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, max_tokens: 600, messages: [{ role: "system", content: "You are the NoteNest writing partner. Return only the requested result." }, { role: "user", content: prompt }] }) });
  ...
  if (provider === "anthropic") {
    response = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "content-type": "application/json", "x-api-key": apiKey, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model, max_tokens: 600, system: "You are the NoteNest writing partner. Return only the requested result.", messages: [{ role: "user", content: prompt }] }) });
    const data = await response.json();
  ...
  if (provider === "google") {
    response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contents: [{ role: "user", parts: [{ text: `You are the NoteNest writing partner. Return only the requested result.\n\n${prompt}` }] }] }) });
    const data = await response.json();
